...
function getSongTitles(songs) {
  ...
  //^--return all desired search w/i () in order for it to be iterated through correctly
...
}//^-- requires same finish as function 2 in test 1 just using .map()
...
function getTitleAndArtist(songs) {
  return songs.map(songs => {
    return { [songs.title]: `${songs.artist}` }
  })
//^-line 46--return array of song titles with array to string song & artist
//^-line 45 to line 47--pay attention to syntax->return {} is for whole returned object & then w/i return {} its the array [] : `${}` which makes an array into a string
}
